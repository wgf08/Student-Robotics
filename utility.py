...
def assess_base_value(b_marker, markers):
    """
    Returns an int "value" representing the acidity/alkalinity of a base.
    ...
    for m in unique_markers:
        ...
        if m.id > 99:
            if cond1 or cond2:
                ...
                v = sample_value(m)
                if v is not None:  # FIX: sample_value can return None for out-of-range IDs
                    value += v
...
def sample_value(marker):
    ...
    if 99 < marker.id < 140 and TARGETED_SAMPLE == 'BASIC' or (139 < marker.id < 200) and TARGETED_SAMPLE == 'ACID':
        return -1
    return 0  # FIX: was implicit None — caused TypeError when caller did `value += sample_value(m)`
